...
export class Store {
    ...
    constructor() {
        ...
        this.images = ['https://file.bgwa.cn/file/demo-img.jpeg',
            'https://file.bgwa.cn/file/thumb.jpg']
    ...
    updateEditorElements(editorElements: EditorElement[]) {
        this.setEditorElements(editorElements)
        this.refreshElements()
    }
    ...
    updateVideoElements() {
        this.editorElements.filter((element): element is VideoEditorElement =>
            element.type === 'video')
            .forEach((element) => {
    ...
    updateAudioElements() {
        this.editorElements.filter((element): element is AudioEditorElement =>
            element.type === 'audio')
            .forEach((element) => {
    ...
    saveCanvasToVideoWithAudioWebmMp4() {
        ...
        videoElements.forEach((video) => {
            const videoElement = document.getElementById(video.properties.elementId) as HTMLAudioElement
            let ctx = new AudioContext()
            var sourceNode = ctx.createMediaElementSource(videoElement)
            let dest = ctx.createMediaStreamDestination()
            sourceNode.connect(dest)
            sourceNode.connect(ctx.destination)
            audioStreams.push(dest.stream)
